...
RSpec.describe ListImport::AnilistV2::Row do
  shared_examples_for 'Anilist V2 generic row fields' do
    describe '#score' do
  ...
  context 'Anime' do
    subject { described_class.new(JSON.parse(media.to_json, object_class: OpenStruct), :anime) }
    ...
    it_behaves_like 'Anilist V2 generic row fields'
  ...
  context 'Manga' do
    subject { described_class.new(JSON.parse(media.to_json, object_class: OpenStruct), :manga) }
