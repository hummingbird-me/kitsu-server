...
RSpec.describe MultiPresenceValidator do
  context 'with a minimum of 1' do
    ...
    MultiPresenceRecordClass = Struct.new(:foo, :bar, :baz) do
      extend ActiveModel::Naming
      include ActiveModel::Validations
    end

    context 'on an invalid record' do
      let(:record) { MultiPresenceRecordClass.new }
    ...
    context 'on a valid record' do
      let(:record) { MultiPresenceRecordClass.new('foo') }
  ...
  context 'with a minimum of 3' do
    ...
    MultiPresenceRecordClass = Struct.new(:foo, :bar, :baz, :bat, :fud) do
      extend ActiveModel::Naming
      include ActiveModel::Validations
    end

    context 'on an invalid record' do
      let(:record) { MultiPresenceRecordClass.new('foo', 'bar') }
      it 'should add an error to each key' do
        subject.validate(record)
    ...
    context 'on a valid record' do
      let(:record) { MultiPresenceRecordClass.new('foo', 'bar', 'baz', 'bat') }
      it 'should have no errors' do
        subject.validate(record)
        expect(record.errors).to be_empty
      end
