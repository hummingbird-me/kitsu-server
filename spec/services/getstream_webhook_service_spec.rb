  ...
  describe '#stringify_activity' do
    context 'follow, post, post_like and comment_like activity' do
      ...
      it 'should localize follow activity string' do
        expect(GetstreamWebhookService.new(request.first).stringify_activity[:en]).to eq("#{actor.name} followed you.")
      end

      it 'should localize post activity string' do
        expect(GetstreamWebhookService.new(request[1]).stringify_activity[:en]).to eq("#{actor.name} mentioned you in a post.")
      end

      it 'should localize post like activity string' do
        expect(GetstreamWebhookService.new(request[2]).stringify_activity[:en]).to eq("#{actor.name} liked your post.")
      end

      it 'should localize comment like activity string' do
        expect(GetstreamWebhookService.new(request[3]).stringify_activity[:en]).to eq("#{actor.name} liked your comment.")
      end
    end
    ...
    context 'comment activity' do
      ...
      context 'when notification feed target and reply to user are same' do
        ...
        it 'should localize reply to post activity string' do
          expect(GetstreamWebhookService.new(post_reply).stringify_activity[:en]).to eq("#{actor.name} replied to your post.")
        end

        it 'should localize reply to comment activity string' do
          expect(GetstreamWebhookService.new(comment_reply).stringify_activity[:en]).to eq("#{actor.name} replied to your comment.")
        end
      end
      ...
      context 'when notification feed target and reply to user are different' do
        ...
        it 'should localize mentioned in a comment activity string' do
          expect(GetstreamWebhookService.new(post_reply).stringify_activity[:en]).to eq("#{actor.name} mentioned you in a comment.")
        end
      end
    end
  end
