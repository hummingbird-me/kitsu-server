...
class AnimeSearchService < TypesenseSearchService
  ...
  def apply_filters_to(scope)
    ...
    scope = apply_auto_filter_for(scope, :average_rating)
    scope = apply_auto_filter_for(scope, :user_count)
    scope = apply_auto_filter_for(scope, :subtype)
    scope = apply_auto_filter_for(scope, :status)
    scope = apply_auto_filter_for(scope, :episode_count)
    scope = apply_auto_filter_for(scope, :episode_length)
    scope = apply_auto_filter_for(scope, :age_rating)
    scope = apply_auto_filter_for(scope, 'start_cour.season', filter_param: :season)
    scope = apply_auto_filter_for(scope, 'start_cour.year', filter_param: :season_year)
    scope = apply_auto_filter_for(scope, 'start_date.year', filter_param: :year)
