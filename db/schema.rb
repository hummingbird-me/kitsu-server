...
ActiveRecord::Schema.define(version: 2024_05_27_222611) do
  ...
  create_table "feed_stories", primary_key: ["feed_id", "story_id"], force: :cascade do |t|
    t.bigint "feed_id", null: false
    t.bigint "story_id", null: false
    t.datetime "bumped_at", null: false
    t.index ["bumped_at"], name: "index_feed_stories_on_bumped_at"
  end
  ...
  create_view "timeline_media", sql_definition: <<-SQL
      ...
      fs.story_id,
      fs.bumped_at
     FROM (( SELECT 'Anime'::text AS media_type,
              anime.id AS media_id,
              anime.feed_id
             FROM anime
          UNION ALL
           SELECT 'Manga'::text AS media_type,
              manga.id AS media_id,
              manga.feed_id
             FROM manga
          UNION ALL
           SELECT 'Drama'::text AS media_type,
              dramas.id AS media_id,
              dramas.feed_id
  ...
  SQL
  create_view "timeline_profile", sql_definition: <<-SQL
      SELECT users.id AS user_id,
      feed_stories.story_id,
      feed_stories.bumped_at
     FROM (feed_stories
       JOIN users ON ((feed_stories.feed_id = users.feed_id)));
  SQL
end
