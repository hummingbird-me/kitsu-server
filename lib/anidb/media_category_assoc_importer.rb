class AnidbAssocMediaCategoryImport
  class ImportFile
    ...
    def initialize(base_media_url, anime_assoc_file, genre_map_file)
      ...
      anime_json_file = open(base_media_url + anime_assoc_file + '?' + utc).read
      @data = JSON.parse(anime_json_file)
      genre_categeory_map = open(base_media_url + genre_map_file + '?' + utc).read
    ...
    def associate_media_categores(media)
      ...
      categories = genres.map { |g| @genre_category_model[g] }.compact if genres.length > 0
      if categories
