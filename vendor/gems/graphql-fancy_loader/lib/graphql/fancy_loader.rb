...
module GraphQL
  class FancyLoader < GraphQL::Batch::Loader
    ...
    # Backwards-compatible configuration block. Replaces ActiveSupport::Configurable
    # (deprecated in Rails 8.1, removed in 8.2). The yielded object is the loader class
    # itself, so existing `config.middleware = ...` callers keep working.
    def self.configure
